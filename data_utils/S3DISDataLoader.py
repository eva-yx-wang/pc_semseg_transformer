...
class S3DISDataset(Dataset):
    def __init__(self, split='train', data_root='trainval_fullarea', num_point=4096, test_area=5, block_size=1.0, sample_rate=1.0, transform=None):
        ...
        for room_name in tqdm(rooms_split, total=len(rooms_split)):
            ...
            # get values of the histogram. eg: data = [1, 2, 1], bin_boundry = [0, 1,2,3] ([0,1), [1,2),[2,3)], np.histogram(data, bins = bin_boundry)
            labelweights += tmp
...
class ScannetDatasetWholeScene():
    ...
    def __init__(self, root, block_points=4096, split='test', test_area=5, stride=0.5, block_size=1.0, padding=0.001):
        ...
        self.block_size = block_size
        self.padding = padding  # conv add 0 around
        self.root = root
        self.split = split
        self.stride = stride
        ...
        for file in self.file_list:
            data = np.load(root + file)  # xyzrgbl
            points = data[:, :3]  # xyz
            self.scene_points_list.append(data[:, :6])  # xyzrgb
            self.semantic_labels_list.append(data[:, 6]) # label
            coord_min, coord_max = np.amin(points, axis=0)[:3], np.amax(points, axis=0)[:3]  # xyz_min, xyz_max in each pc_file
            self.room_coord_min.append(coord_min), self.room_coord_max.append(coord_max)
    ...
    def __getitem__(self, index):
        ...
        coord_min, coord_max = np.amin(points, axis=0)[:3], np.amax(points, axis=0)[:3]
        grid_x = int(np.ceil(float(coord_max[0] - coord_min[0] - self.block_size) / self.stride) + 1)  # sence_size of axis x
        grid_y = int(np.ceil(float(coord_max[1] - coord_min[1] - self.block_size) / self.stride) + 1)  # sence_size of axis y
