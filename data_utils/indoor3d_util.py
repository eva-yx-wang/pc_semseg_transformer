...
DATA_PATH = "/data/lxr/S3DIS/Stanford3dDataset_v1.2_Aligned_Version"
...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    ...
    for f in sorted(glob.glob(os.path.join(anno_path, '*.txt'))):
        ...
        points = np.loadtxt(f)  # xyzrbg
        labels = np.ones((points.shape[0],1)) * g_class2label[cls]  # label
        points_list.append(np.concatenate([points, labels], 1)) # N * (xyzrbgl)
    
    data_label = np.concatenate(points_list, 0)
    xyz_min = np.amin(data_label, axis=0)[0:3]  # take minimum along axis = 0 (xyzrgbl)
    data_label[:, 0:3] -= xyz_min  # substract min of xyz
